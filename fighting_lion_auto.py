...
def stop_script(e):
    subprocess.run('netsh advfirewall firewall delete rule name="Destiny2-Solo-1"', shell=True)
    subprocess.run('netsh advfirewall firewall delete rule name="Destiny2-Solo-2"', shell=True)
    subprocess.run('netsh advfirewall firewall delete rule name="Destiny2-Solo-3"', shell=True)
    subprocess.run('netsh advfirewall firewall delete rule name="Destiny2-Solo-4"', shell=True)
    global running
    running = False
...
# Remove the 27k (in case there's any)
subprocess.run('netsh advfirewall firewall delete rule name="Destiny2-Solo-1"', shell=True)
subprocess.run('netsh advfirewall firewall delete rule name="Destiny2-Solo-2"', shell=True)
subprocess.run('netsh advfirewall firewall delete rule name="Destiny2-Solo-3"', shell=True)
subprocess.run('netsh advfirewall firewall delete rule name="Destiny2-Solo-4"', shell=True)
...
# Block 27k ports
subprocess.run('netsh advfirewall firewall add rule name="Destiny2-Solo-1" dir=out remoteport=27000-27100 protocol=tcp action=block', shell=True)
subprocess.run('netsh advfirewall firewall add rule name="Destiny2-Solo-2" dir=out remoteport=27000-27100 protocol=udp action=block', shell=True)
subprocess.run('netsh advfirewall firewall add rule name="Destiny2-Solo-3" dir=in localport=27000-27100 protocol=tcp action=block', shell=True)
subprocess.run('netsh advfirewall firewall add rule name="Destiny2-Solo-4" dir=in localport=27000-27100 protocol=udp action=block', shell=True)

win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTDOWN, 0, 0)
time.sleep(.3)
ref_img = ImageGrab.grab(bbox=(x_start, y_start, x_end, y_end)).convert("L")  # X1,Y1,X2,Y2
# ref_img = ref_img.save("ref_img.png") # Saves the reference image
